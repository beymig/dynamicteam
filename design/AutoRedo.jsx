...
var Task = function(source){
  ...
  this.export_all = function(){
    ...
    for (var i=0; i<folder_count; i++) {
      ...
      var ok = sub_folder.rename(OUTPUT_FOLDER + "\\" + [COMBINED_PREFIX, folder_id, sub_folders[folder_id].toString()+"in", (i+1).toString()+"-"+folder_count.toString(), source.id].join('_'));
      if(!ok)
...
function main(){
	...
	var ptthelper = new ExternalObject("lib:d:\\workspace\\dynamicteam\\src\\design\\ptthelper.dll");
    CUTCODE_TEXTFRAME = createTitleText();

  while(1){
    var redoInfo = ptthelper.getANewRedo().split(';');
    if (redoInfo.length < 5){
      $.sleep(10*1000);
      continue;
    }
    var redo = new Object();
    redo.id = redoInfo.shift();
    redo.log = redoInfo.shift();
    redo.department = redoInfo.shift();
    redo.create_at = redoInfo.shift();
    redo.pieces = redoInfo;

    $.writeln(redoInfo.join(','));

    ptthelper.updateRedoStatus(parseInt(redo.id), "building");

    var task = new Task(redo);
    task.export_all();
    ptthelper.updateRedoStatus(parseInt(redo.id), "built");
